...
process_directory() {
  ...
  for file in "${files[@]}"; do
    ...
    if [[ -n $alias_line ]]; then
      ...
      if grep -q '^#MS_SUDO' "$file"; then
        alias_need_sudo["$alias_name"]=true
      fi
...
for alias_name in "${!alias_to_file[@]}"; do
  ...
  if (( verbosity >= 2 )); then
    ...
    [[ "${alias_is_system[$alias_name]}" == "true" ]] && display_name+=" (system)"
    [[ "${alias_need_sudo[$alias_name]}" == "true" ]] && display_name+=" (sudo)"
